"""
Backtest regime filter and increased lookback window for cascade detection.

Variations:
1. Baseline: liq_lookback=90, liq_percentile=0.90 (current)
2. Longer lookback: liq_lookback=180, liq_percentile=0.90
3. Regime filter: liq_lookback=90, liq_percentile=0.90, min_regime_ratio=0.30
   (suppress cascade if recent 10-day mean liq < 30% of full lookback mean)
4. Both: liq_lookback=180, liq_percentile=0.90, min_regime_ratio=0.30
"""
...
from agg_exit_v2_backtest import (
    ...
    compute_aggression, load_all_data, _atr, _ema, _z_score
)

# V4 exit params (7-tuple)
EXIT_PARAMS_V4 = {
...
class CascadeTrackerRegime:
    """Cascade tracker with configurable lookback and regime filter."""
    def __init__(self, liq_lookback=90, liq_percentile=0.90, min_regime_ratio=0.0):
        self._liq_history = deque(maxlen=liq_lookback + 5)
        self._liq_lookback = liq_lookback
        self._liq_percentile = liq_percentile
        self._min_regime_ratio = min_regime_ratio
    ...
    def update(self, daily_row):
        ...
        lookback = liqs[-self._liq_lookback:] if len(liqs) >= self._liq_lookback else liqs
        ...
        # Regime filter: suppress if recent liq is too low vs historical
        if self._min_regime_ratio > 0 and len(liqs) >= 30:
            recent_mean = np.mean(liqs[-10:]) if len(liqs) >= 10 else np.mean(liqs)
            full_mean = np.mean(liqs)
            if full_mean > 0 and recent_mean / full_mean < self._min_regime_ratio:
                cascade_active = False
...
def run_backtest_regime(klines, daily_liq, daily_closes, liq_lookback=90, liq_percentile=0.90, min_regime_ratio=0.0):
    """Backtest with regime-filtered cascade tracker and V4 exits."""
    daily_liq_by_date = {}
    ...
    for idx, (sym, bar) in enumerate(all_bars):
        if sym not in states:
            ...
            cascade_trackers[sym] = CascadeTrackerRegime(liq_lookback, liq_percentile, min_regime_ratio)
        ...
        if sym in current_trades:
            ...
            if low_r < tc['mae']: tc['mae'] = low_r
            if high_r > tc['mfe']: tc['mfe'] = high_r
            if price <= bar['open']:
            ...
            c_arr = np.array([c['close'] for c in candles[-50:]])
            atr = _atr(h, l, c_arr, CFG.atr_period)
            ...
            if bar['low'] <= stop_price:
                tc['exit_reason'] = 'stop_loss'; tc['exit_price'] = stop_price
                tc['pnl_r'] = (stop_price - tc['entry_price']) / tc['risk_per_unit']
                lifecycles.append(tc); del current_trades[sym]
                st['in_trade'] = False; st['stopped_in_window'] = True; st['cooldown'] = CFG.cooldown_bars
                continue
            ...
            if new_vol_trail > tc['vol_trail']: tc['vol_trail'] = new_vol_trail
            if tc['vol_trail'] > tc['entry_price'] and bar['low'] <= tc['vol_trail']:
                tc['exit_reason'] = 'vol_trail'; tc['exit_price'] = tc['vol_trail']
                tc['pnl_r'] = (tc['vol_trail'] - tc['entry_price']) / tc['risk_per_unit']
                lifecycles.append(tc); del current_trades[sym]
                st['in_trade'] = False; st['cooldown'] = CFG.cooldown_bars
                continue

            if len(candles) >= struct_lb:
                swing_low = min(c['low'] for c in candles[-struct_lb:])
                if swing_low > tc['struct_trail']: tc['struct_trail'] = swing_low
                if tc['struct_trail'] > tc['entry_price'] and bar['low'] <= tc['struct_trail']:
                    tc['exit_reason'] = 'struct_trail'; tc['exit_price'] = tc['struct_trail']
                ...
                if tc['bars_held'] >= decay_bars and current_r > 0.5:
                    ...
                    if peak_r >= decay_mfe and tc['consecutive_red'] >= decay_red:
                        ...
                        if pullback_r >= decay_pullback:
                            tc['exit_reason'] = 'expansion_decay'; tc['exit_price'] = price
                            tc['pnl_r'] = current_r
                            lifecycles.append(tc); del current_trades[sym]
                            st['in_trade'] = False; st['cooldown'] = CFG.cooldown_bars
                            continue
            ...
            if tc['bars_held'] >= max_hold:
                tc['exit_reason'] = 'time_stop'; tc['exit_price'] = price
                tc['pnl_r'] = current_r
                lifecycles.append(tc); del current_trades[sym]
                st['in_trade'] = False; st['cooldown'] = CFG.cooldown_bars
                continue
        ...
        # Entry logic
        if st['cooldown'] > 0:
            st['cooldown'] -= 1; continue
        if st['in_trade'] or not st['cascade_active']: continue
        if st['cooldown'] > 0: continue
        if CFG.no_reentry_after_stop and st['stopped_in_window']: continue

        candles = sym_candles[sym]
        n_needed = max(CFG.range_lookback, CFG.z_lookback, CFG.ema_period * 3)
        if len(candles) < n_needed: continue
        ...
        if confirm_count < CFG.min_confirmations: continue
        ...
        current_trades[sym] = {
            'symbol': sym, 'entry_time': bar['close_time'],
            'entry_price': entry_price, 'stop_price': stop_price,
            'risk_per_unit': stop_distance, 'bars_held': 0,
            'partial_taken': False, 'best_price': entry_price,
            'vol_trail': 0, 'struct_trail': 0, 'mae': 0, 'mfe': 0,
            'consecutive_red': 0, 'aggression': aggression, 'decile': decile,
            'quantity': 1.0, 'rpnl': 0.0,
...
def analyze_results(trades, label):
    ...
    for t in trades: by_decile[t['decile']].append(t['pnl_r'])
...
print(f"Loaded: {sum(len(v) for v in klines.values())} klines, {len(daily_liq)} symbols")

configs = [
    (90,  0.90, 0.00, "Baseline (lookback=90, p90)"),
    (180, 0.90, 0.00, "Longer lookback (180d, p90)"),
    (90,  0.90, 0.30, "Regime filter (90d, p90, min_ratio=0.30)"),
    (90,  0.90, 0.50, "Regime filter (90d, p90, min_ratio=0.50)"),
    (180, 0.90, 0.30, "Both (180d, p90, min_ratio=0.30)"),
    (180, 0.95, 0.00, "Longer + stricter pct (180d, p95)"),
]

results = {}
for lb, pct, ratio, label in configs:
    print(f"\nRunning: {label}...")
    trades = run_backtest_regime(klines, daily_liq, daily_closes, lb, pct, ratio)
    results[label] = analyze_results(trades, label)

print(f"\n{'='*70}\nCOMPARISON SUMMARY\n{'='*70}")
print(f"{'Config':<50} {'Trades':<8} {'WR%':<8} {'Total R':<10} {'PF':<8} {'NEAR R':<10}")
print("-" * 95)
for lb, pct, ratio, label in configs:
    r = results[label]
    if r:
        print(f"{label:<50} {r['n']:<8} {r['wr']:<8.1f} {r['total_r']:<+10.2f} {r['pf']:<8.2f} {r['near_r']:<+10.2f}")
