...
| ADX hysteresis + 5-min gate refresh — **PREREG-ADXBAND — IMPLEMENTED, verification window running** | Bot side shipped 2026-09-03 (owner-authorized): `_refresh_btc_regime` deadband 25.5-in/24.5-out (main.py 540) + 300s refresh (main.py 373) — plan row was stale "pending Martin", nothing outstanding. Cron side: notifier v2 RELAY mode Sep-12 (commit 5a5c27c) — script recomputed its own deadband on 15m cadence and diverged from the bot (fired neutral 00:15Z vs bot 04:01Z Sep-12; relay = zero divergence by construction) | 7 days from relay commit (~Sep-19 Sunday loop): every bot transition mirrored within one cycle; revert criteria moot unless relay breaks |
