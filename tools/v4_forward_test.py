"""
Bitana V4 Forward Test — Liq-Cluster Multi-Symbol Paper Trading.
...
Revert: run tools/v3_forward_test.py instead.
"""
...
logger = get_logger("v4_forward_test")

# ═══════════════════════════════════════════════════
# Config Loading
# ═══════════════════════════════════════════════════

CONFIG_PATH = Path(__file__).parent.parent / "config" / "v4_forward_test.yaml"

def load_v4_config() -> dict:
...
TAKER_BPS = 4.5
SLIP_BPS = 2.0
DB_PATH = Path("storage/v4_forward_test.db")
DAILY_REPORT_HOUR = 8
...
class V4ForwardTest:
    def __init__(self):
        ...
        self.v4_cfg = load_v4_config()
        self.db = V4Database(DB_PATH)
        self.engine = LiqClusterEngineV4()
        ...
        # Symbols
        self.symbols = self.v4_cfg["symbols"]["tier_a"] + self.v4_cfg["symbols"]["tier_b"]
        ...
        # Coinalyze
        self.ca_api_key = self.v4_cfg["coinalyze"]["api_key"]

        # State
        saved_eq = self.db.get_state("equity")
        eq = float(saved_eq) if saved_eq else self.v4_cfg.get("initial_equity", 10000.0)
    ...
    async def start(self):
        ...
        if self.open_positions:
            ...
            for p in self.open_positions:
                sym = p["symbol"]
                # Restore engine state for recovered position
                ...
                # Recompute aggression from candle buffer for fresh decile
                if sym in self.candle_buffers and len(self.candle_buffers[sym]) >= 25:
                    from engines.liq_cluster_engine_v4 import _compute_aggression, _score_to_decile
                    agg = _compute_aggression(self.candle_buffers[sym])
                    dec = _score_to_decile(agg)
                    p["aggression"] = agg
                    p["decile"] = dec
                    st.aggression_score = agg
                    st.decile = dec
                    logger.info("Recomputed aggression for recovered position",
                               symbol=sym, aggression=round(agg, 1), decile=dec)
        ...
        try:
            ...
            for i, r in enumerate(results):
                if isinstance(r, Exception):
                    ...
                    await self.alerts.critical(f"V4 task crashed: {name}: {r}")
        finally:
            await self._cleanup()
    ...
    async def _manage_positions(self, symbol: str, candle: Candle):
        ...
        for p in sym_positions:
            ...
            if result and result["action"] == "partial" and not p["tp1_hit"]:
                ...
                r_at = result.get("r", 0)
                agg = p.get("aggression", 0)
                dec = p.get("decile", 0)
                await self.alerts.send(
                    f"💰 V4 PARTIAL {symbol}\n"
                    f"50% off at {r_at:.1f}R\n"
                    f"PnL: {pnl:+.2f}\n"
                    f"Aggression: {agg:.0f} (D{dec})",
                    AlertTier.INFO,
                )
    ...
    async def _close_position(self, p, price, reason, ct):
        ...
        mae = p.get('mae', 0)
        mfe = p.get('mfe', 0)
        candles = p['candles_held']
        eq = self.executor.equity
        msg = (
            f"{emoji} V4 EXIT {side} {sym}\n"
            f"Entry: {entry_p:.6f} → Exit: {fill:.6f}\n"
            f"PnL: {net_pnl:+.2f} ({pnl_r:+.3f}R)\n"
            f"Reason: {reason} | Candles: {candles}\n"
            f"MAE: {mae:.2f}R | MFE: {mfe:.2f}R\n"
            f"Aggression: {agg:.0f} (D{dec})\n"
            f"BTC: {aligned} | Eq: {eq:.2f}"
        )
    ...
    async def _check_entry(self, symbol: str, candle: Candle):
        ...
        dec = sig.signal_data.get("decile", 0)
        await self.alerts.send(
            f"🔍 V4 SIGNAL {symbol}\n"
        ...
        pos = {
            ...
            "mae": 0.0,
            "mfe": 0.0,
            "aggression": sig.signal_data.get("aggression_score", 0),
            "decile": sig.signal_data.get("decile", 0),
        }
        ...
        logger.info("Entry", symbol=symbol, price=round(fill, 6),
                     ...
                     aggression=round(agg, 1), decile=dec)
...
async def async_main():
    setup_logging(level="INFO", log_file="logs/v4_forward_test.log")
