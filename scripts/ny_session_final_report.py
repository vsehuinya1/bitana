#!/usr/bin/env python3
"""
NY Session FINAL Live Trade Report — runs at 01:00 UTC (4h after NY close)
Shows COMPLETE session PnL including trades that were still open at 21:00 report.
"""
import sqlite3
import sys
...
def fetch_all_session_trades(conn, start_utc, end_utc):
    """Fetch ALL trades where signal_time falls in NY window (closed + open)."""
    query = """
...
def main():
    ...
    print(f"=== NY SESSION FINAL REPORT ===")
    ...
    print(f"ALL TRADES (NY session): {len(all_trades)}")
    total_r = 0.0
    if all_trades:
        by_symbol = {}
    ...
    print(f"\n  SESSION TOTAL R (NY): {format_r(total_r)}")
