#!/usr/bin/env python3
"""
NY Session Live Trade Report — runs at 17:00 and 22:00 UTC
Lists closed trades from NY session (13:00-21:00 UTC) by pair and R,
...
def get_ny_window():
    """Return (start_utc, end_utc) for today's NY session 13:00-21:00 UTC."""
    now = datetime.now(timezone.utc)
    today = now.date()
    start = datetime(today.year, today.month, today.day, 13, 0, 0, tzinfo=timezone.utc)
    end = start + timedelta(hours=8)
    return start, end

def fetch_closed_ny_trades(conn, start_utc, end_utc):
    """Fetch trades where signal_time falls in NY window."""
    query = """
...
def main():
    start_utc, end_utc = get_ny_window()
    now_utc = datetime.now(timezone.utc)
    
    print(f"=== NY SESSION REPORT ===")
    ...
    # Closed trades in NY session
    closed = fetch_closed_ny_trades(conn, start_utc, end_utc)
    
    print(f"CLOSED TRADES (NY session): {len(closed)}")
    total_r = 0.0
    if closed:
        by_symbol = {}
        for row in closed:
        ...
        print("  (none)")
    print(f"\n  TOTAL R (NY session): {format_r(total_r)}")
